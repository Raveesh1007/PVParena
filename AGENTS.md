...
- Benchmark: **`Equity.US.NVDA/USD`**, exponent **−5**, Spot, 24/5, through authenticated Hermes and

  the Solana receiver. If the key cannot reach it, walk the `code.md` §3.2 fallback tree

  (NVDA → AAPL → AAPLX → AAPLON → stop and report). **A generic crypto feed is never an acceptable
...
- **Terminal feed IDs are not Core feed IDs.** 1314 (NVDA) and 922 (AAPL) are Pro/Lazer identifiers.
...
- Prove benchmark access and one devnet post/read before building the full oracle path. The feed is

  between sessions on weekends (Sat 05:30 – Mon 05:30 IST) — the first live test is Monday 21 Sep
